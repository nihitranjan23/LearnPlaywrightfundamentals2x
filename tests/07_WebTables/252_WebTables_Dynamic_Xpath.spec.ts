...
test('How to handle webtabel  ', async ({ page }) => {
    ...
    const rows = await page.locator("//table[@id='customers']/tbody/tr").count(); //count the number of rows
    const cols = await page.locator("//table[@id='customers']/tbody/tr[2]/td").count();  //count the number of columns
